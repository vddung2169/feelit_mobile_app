...
final class OTPInputView: UIView {
    ...
    private func update() {
        ...
        for (i, box) in boxes.enumerated() {
            ...
            else { color = AuthTheme.fieldBorder }
...
final class AuthOTPViewController: UIViewController {
    ...
    /// Mã đúng để demo (thực tế cần backend gửi & xác minh).
    private let demoCorrectCode = "123456"
    ...
    // MARK: Subtitle (mask số điện thoại)
    private func makeSubtitle() -> NSAttributedString {
        ...
        s.append(NSAttributedString(string: maskedPhone(), attributes: [
            .font: UIFont.systemFont(ofSize: 16, weight: .regular),
    ...
    private func maskedPhone() -> String {
        let parts = phoneNumber.split(separator: " ", maxSplits: 1).map(String.init)
        guard parts.count == 2 else { return phoneNumber }
        let dial = parts[0]
    ...
    private func verify(_ code: String) {
        if code == demoCorrectCode {
            // TODO: xác minh OTP với backend; ở đây demo chấp nhận mã "123456".
            view.endEditing(true)
            navigationController?.pushViewController(
                AuthSuccessViewController(email: phoneNumber), animated: true)
        } else {
    ...
    @objc private func resendTapped() {
        ...
        startCountdown()
        otpView.becomeFirstResponder()
        // TODO: gọi backend gửi lại mã OTP.
    }
